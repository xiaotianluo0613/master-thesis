#!/usr/bin/env python3
"""
Build Layer 3 chunk dataset for full-scale training.

Chunk unit: one XML page with metadata prefix.
Sampling: single type (Legal) — take all pages up to target.
...
def make_quotas(total: int, available: Dict[str, int]) -> Dict[str, int]:
    """
    Single-type sampling: take all Legal pages up to target.
    """
    return {
        "Legal": min(available.get("Legal", 0), total),
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Build Layer 3 chunks for full-scale training")
    parser.add_argument("--pool", default="output/data_pools/train_layer3_pool.txt")
    ...
    parser.add_argument("--output", default="data/layer3_chunks.json")
    ...
    out = {
        "metadata": {
            "dataset": "layer3_chunks",
            "target_chunks": args.target_chunks,
            "actual_chunks": len(chunks),
            "target_types": TARGET_TYPES,
            "quotas": quotas,
            "sampling": "single type — all Legal pages up to target",
