#!/bin/bash
# Layer 3 query generation — runs on UPPMAX LOGIN NODE (not sbatch).
# Login nodes have internet access; compute nodes do not.
#
# Generates N-to-N queries via Gemini for Layer 3 chunks.
# Uses --resume so it is safe to re-run if interrupted.
#
# Input:  data/layer3_chunks_grouped.json  (scp'd from local Mac)
# Output: data/layer3_queries.json
#
# Usage (run directly on login node, not via sbatch):
#   export GEMINI_API_KEY=<your-key>
#   nohup bash slurm/layer3_query_gen.sh > logs/layer3_query_gen.log 2>&1 &
#   tail -f logs/layer3_query_gen.log
...
echo "=== Layer 3 query generation ==="
echo "Input:  data/layer3_chunks_grouped.json"
echo "Output: data/layer3_queries.json"
echo "Started: $(date)"
...
echo "Done: data/layer3_queries.json"
echo "Finished: $(date)"
echo "Next: sbatch slurm/layer3_split.sh"
