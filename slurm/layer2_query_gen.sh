#!/bin/bash
# Layer 2 query generation — runs on UPPMAX LOGIN NODE (not sbatch).
# Login nodes have internet access; compute nodes do not.
#
# Generates N-to-N queries via Gemini for Layer 2 chunks.
# Uses --resume so it is safe to re-run if interrupted.
#
# Input:  data/layer2_chunks_grouped.json  (scp'd from local Mac)
# Output: data/layer2_queries.json
#
# Usage (run directly on login node, not via sbatch):
#   export GEMINI_API_KEY=<your-key>
#   nohup bash slurm/layer2_query_gen.sh > logs/layer2_query_gen.log 2>&1 &
#   tail -f logs/layer2_query_gen.log
...
echo "=== Layer 2 query generation ==="
echo "Input:  data/layer2_chunks_grouped.json"
echo "Output: data/layer2_queries.json"
echo "Started: $(date)"
...
echo "Done: data/layer2_queries.json"
echo "Finished: $(date)"
echo "Next: sbatch slurm/layer2_split.sh"
