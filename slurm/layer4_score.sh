...
#SBATCH -N 1
#SBATCH --mem=32G
#SBATCH -J layer4_score
#SBATCH -o logs/layer4_score_%j.out
#SBATCH -e logs/layer4_score_%j.err
...
# Input:  output/layer4_bge_negatives.json
# Output: output/layer4_bge_training_data_scored.jsonl
...
python scripts/pipeline/convert_to_flagembedding_format.py \
    --input  output/layer4_bge_negatives.json \
    --output output/layer4_bge_training_data.jsonl

python scripts/pipeline/score_bge_integration.py \
    --input      output/layer4_bge_training_data.jsonl \
    --output     output/layer4_bge_training_data_scored.jsonl \
    --model      BAAI/bge-m3 \
    --batch-size 32

echo "Done: output/layer4_bge_training_data_scored.jsonl"
