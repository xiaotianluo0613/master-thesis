...
#SBATCH -N 1
#SBATCH --mem=32G
#SBATCH -J layer2_score
#SBATCH -o logs/layer2_score_%j.out
#SBATCH -e logs/layer2_score_%j.err
...
# Input:  output/layer2_bge_negatives.json
# Output: output/layer2_bge_training_data_scored.jsonl
...
python scripts/pipeline/convert_to_flagembedding_format.py \
    --input  output/layer2_bge_negatives.json \
    --output output/layer2_bge_training_data.jsonl

python scripts/pipeline/score_bge_integration.py \
    --input      output/layer2_bge_training_data.jsonl \
    --output     output/layer2_bge_training_data_scored.jsonl \
    --model      BAAI/bge-m3 \
    --batch-size 32

echo "Done: output/layer2_bge_training_data_scored.jsonl"
