#!/usr/bin/env python3
"""
Build Layer 4 chunk dataset for full-scale training.

Chunk unit: one XML page with metadata prefix.
Sampling: single type (City) — take all pages up to target.
...
def make_quotas(total: int, available: Dict[str, int]) -> Dict[str, int]:
    """
    Single-type sampling: take all City pages up to target.
    """
    return {
        "City": min(available.get("City", 0), total),
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Build Layer 4 chunks for full-scale training")
    parser.add_argument("--pool", default="output/data_pools/train_layer4_pool.txt")
    ...
    parser.add_argument("--target-chunks", type=int, default=1500)
    parser.add_argument("--min-text-chars", type=int, default=220)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--output", default="data/layer4_chunks.json")
    ...
    quotas = make_quotas(args.target_chunks, available)
    ...
    out = {
        "metadata": {
            "dataset": "layer4_chunks",
            "target_chunks": args.target_chunks,
            "actual_chunks": len(chunks),
            "target_types": TARGET_TYPES,
            "quotas": quotas,
            "sampling": "single type — all City pages up to target",
