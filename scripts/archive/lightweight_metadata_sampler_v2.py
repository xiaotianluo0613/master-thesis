#!/usr/bin/env python3
"""
Lightweight Archive Metadata Sampler - Version 2
Simplified and more robust for large-scale sampling.
Uses direct file reading instead of heavy XML iteration.
"""
...
TITLE_KEYWORDS = {'Rapporter', 'Protocoll', 'Domböcker', 'Index', 'Register', 'Journaler'}
YEAR_PATTERN = re.compile(r'(?:år\s*)?(\d{4})')
PC_PATTERN = re.compile(r'PC="([0-9.]+)"')


class LightweightSamplerV2:
    """Fast metadata sampler - simplified version."""
    ...
    def read_file_text_simple(self, filepath: Path, max_chars: int = 3000) -> str:
        """Read XML file as plain text (much faster and safer than parsing)."""
        try:
            with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                text = f.read(max_chars * 2)  # Read more to account for XML tags
    ...
    def get_pc_score_simple(self, filepath: Path) -> Optional[float]:
        """Extract PC score from file without full parsing."""
        try:
            with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                text = f.read(5000)
    ...
    def find_title_and_type(self, volume_path: Path) -> tuple:
        """Find title, year, and document type by scanning all files in volume."""
        xml_files = sorted(volume_path.glob("*.xml"))
        
        # Scan up to 10 files to find content with keywords
        for xml_file in xml_files[:10]:
            text = self.read_file_text_simple(xml_file)
            
            # Look for document type keywords anywhere in volume
        ...
        # If no keyword found, return generic based on file count
        ...
        try:
            ...
            if doc_type not in self.seen_types:
                ...
                if xml_files:
                    ...
                    sample_text = self.read_file_text_simple(sample_file, max_chars=2000)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Lightweight metadata sampler (fast, safe version)'
    )
    ...
    sampler = LightweightSamplerV2(root_path)
