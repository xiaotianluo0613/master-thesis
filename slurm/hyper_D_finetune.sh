...
#SBATCH --mem=64G
#SBATCH -J hyper_D
#SBATCH -o logs/hyper_D_%j.out
#SBATCH -e logs/hyper_D_%j.err

# Hyperparameter tuning — Run D
# LR: 3e-5 | Scheduler: cosine | Epochs: 2
...
mkdir -p logs output/models/hyper_D
...
torchrun --nproc_per_node 1 \
    ...
    --output_dir                output/models/hyper_D \
    --train_group_size          8 \
    --query_max_len             512 \
    --passage_max_len           512 \
...
echo "Done: output/models/hyper_D"

echo "=== Merging hyper_D LoRA checkpoint ==="
python scripts/pipeline/merge_lora_checkpoint.py \
    --adapter-dir output/models/hyper_D \
    --output-dir  output/models/hyper_D-merged \
    --base-model  BAAI/bge-m3

echo "Done: output/models/hyper_D-merged"
