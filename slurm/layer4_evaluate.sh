...
#SBATCH -N 1
#SBATCH --mem=64G
#SBATCH -J layer4_eval
#SBATCH -o logs/layer4_eval_%j.out
#SBATCH -e logs/layer4_eval_%j.err

# Layer 4 evaluation — two passes:
#   Global: 1150 queries (all 4 layers), 13800-chunk corpus
#   Layer-specific: L4 val queries (150), L4 corpus only (1500 chunks)
# Step 1: merge L4 LoRA adapter into full checkpoint
# Step 2: build combined corpus (L1+L2+L3+L4, 13800 chunks) if not already built
# Step 3: evaluate baseline -> L1 -> L2 -> L3 -> L4 on global val
# Step 4: evaluate baseline -> L1 -> L2 -> L3 -> L4 on L4 val only
# Output: output/layer4_eval_results.json, output/layer4_eval_layer_specific_results.json
...
mkdir -p logs output output/models/layer4-bge-m3-lora-dense-b4-merged

module load Python/3.11.5-GCCcore-13.3.0
source .venv/bin/activate

# Step 1: merge L4 LoRA checkpoint
...
python scripts/pipeline/evaluate_comparison.py \
    ...
    --k-values   10 100 \
    --batch-size 64 \
    --output     output/layer4_eval_results.json

echo "Done: output/layer4_eval_results.json"

# Step 4: layer-specific eval
echo "=== Layer 4-specific evaluation ==="
python scripts/pipeline/evaluate_comparison.py \
    --chunks     data/layer4_chunks_grouped.json \
    --queries    data/layer4_val_queries.json \
    --models     BAAI/bge-m3 \
                 output/models/layer1-bge-m3-lora-dense-b4-merged \
                 output/models/layer2-bge-m3-lora-dense-b4-merged \
                 output/models/layer3-bge-m3-lora-dense-b4-merged \
    ...
    --k-values   10 100 \
    --batch-size 64 \
    --output     output/layer4_eval_layer_specific_results.json

echo "Done: output/layer4_eval_layer_specific_results.json"
