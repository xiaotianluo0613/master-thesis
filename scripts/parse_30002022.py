#!/usr/bin/env python3
"""
ALTO XML parser for 30002022 folder - extracts text and converts to JSONL format.
...
def parse_alto_file(filepath: Path) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Get folder name as volume metadata (30002022)
...
def main():
    """
    Main function to parse all ALTO XML files from 30002022 and write to JSONL.
    """
    workspace_root = Path(__file__).parent
    
    # Find all XML files in 30002022 folder
    xml_files = sorted(workspace_root.glob("Filedrop-7hXHfBBqt2nJHQuk/home/dgxuser/erik/data/transcriptions/30002022/*.xml"))
    
    if not xml_files:
        print("No XML files found in 30002022.", file=sys.stderr)
        sys.exit(1)
    
    # Output file
    output_file = workspace_root / "output_30002022.jsonl"
    
    # Parse and write to JSONL
    with open(output_file, "w", encoding="utf-8") as outf:
        for filepath in tqdm(xml_files, desc="Parsing 30002022 ALTO files"):
