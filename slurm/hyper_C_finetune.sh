...
#SBATCH --mem=64G
#SBATCH -J hyper_C
#SBATCH -o logs/hyper_C_%j.out
#SBATCH -e logs/hyper_C_%j.err

# Hyperparameter tuning — Run C
# LR: 3e-5 | Scheduler: cosine | Epochs: 3
...
mkdir -p logs output/models/hyper_C
...
torchrun --nproc_per_node 1 \
    ...
    --output_dir                output/models/hyper_C \
    --train_group_size          8 \
    --query_max_len             512 \
    --passage_max_len           512 \
...
echo "Done: output/models/hyper_C"

echo "=== Merging hyper_C LoRA checkpoint ==="
python scripts/pipeline/merge_lora_checkpoint.py \
    --adapter-dir output/models/hyper_C \
    --output-dir  output/models/hyper_C-merged \
    --base-model  BAAI/bge-m3

echo "Done: output/models/hyper_C-merged"
